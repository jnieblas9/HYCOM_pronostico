...
PRECVALUES = [50, 50, 50, 50, 95, 99, 99.9]#el orden si importa
...
def get_file_day(dom, path):
    ...
    nfile = path + TOKENFILE + "wrfout_d0" \
...
def alert_values(forecast, percentiles, percentilesValues, percentilesVars):
    ...
    for dayforecast in forecast:#itera sobre las horas
        i = 0
        for ky in percentilesVars:#sobre las llaves oredenadas de los percentiles
...
def alert_t2(spath, dom, fpath, ppath):
    ...
    forecast = get_var_netcdf("T2", netcdf) - 273.15#se cambia de grados kelvin a celsius
    ...
    alertMax = np.amax(alert, axis=0)
    alertMin = np.amin(alert, axis=0)
    alertMax[alertMax <= 50] = alertMin[alertMax<=50]
    var.update(netcdf_var_t2(alert, np.array([alertMax])))
...
def alert_prec(spath, dom, fpath, ppath):
    ...
    percentiles = get_file_percentiles(PRECVARS, dom, ppath)
...
#def estep2(path, pvars,  lat, lon, name):
    ...
    #for var in pvars:
        ...
        #f = interpolate.interp2d(x, y, np.transpose(vvar), kind='cubic')
