...
source $RUTA_SISTEMA/scripts/bash/configuracion/variables.sh
...
actualizar_registro "ESTOY en EJECUTA ADCIRC"
actualizar_registro "La RAIZ_SISTEMA = "$RAIZ_SISTEMA
actualizar_registro "La RUTA_SISTEMA = "$RUTA_SISTEMA

# Actualizar los registros de eventos
actualizar_registro "*** INICIO DEL SCRIPT DE EJECUCION DEL MODELO ADCIRC PARA EL DOMINIO "$DOMINIO
...
actualizar_registro  "$RUTA_SISTEMA/modelos/ADCIRC/pewap/pewap4.exe -b $RUTA_SISTEMA/modelos/WRF/WRFV3/test/em_real/$SALIDA_WRF -p nodefile_$DOMINIO"
...
actualizar_registro "La ejecucion del pewap4.exe tardo:"`cat tiempo.txt`" (h:m:s)"
if [ $estado_programa -ne 0 ]; then
        # Actualizar los registros de eventos
        actualizar_registro "***ERROR*** No se pudo generar el archivo de vientos para adcirc en el dominio "$DOMINIO
...
actualizar_registro "VOY A CAMBIAR LA FECHA A"$NCFECHA
sed "s/FECHA_ACTUAL/${NCFECHA}/g" fort.15 > fort.15.tmp && mv fort.15.tmp fort.15

# Ejecutar adcprep para dividir los datos de viento

$RUTA_SISTEMA/modelos/ADCIRC/adcirc-v52.30.13_pgi/work/adcprep --np $CORES --partmesh
/usr/bin/time --format="%E" --output=tiempo.txt $RUTA_SISTEMA/modelos/ADCIRC/adcirc-v52.30.13_pgi/work/adcprep --np $CORES --prepall
actualizar_registro "La ejecucion del segundo adcprep tardo:"`cat tiempo.txt`" (h:m:s)"
...
$RUTA_SISTEMA/modelos/ADCIRC/adcirc-v52.30.13_pgi/work/adcprep <<END_SCRIPT
$CORES
$opcion
$ihot
END_SCRIPT
...
actualizar_registro "Inicio de la ejecucion de padcirc para el dominio "$DOMINIO
...
/usr/bin/time --format="%E" --output=tiempo.txt mpirun -np $CORES --mca mpi_cuda_support 0 $RUTA_SISTEMA/modelos/ADCIRC/adcirc-v52.30.13_pgi/work/padcirc

# Verificar si el modelo se ejecuto exitosamente
estado_modelo=$?

actualizar_registro "La ejecucion del padcirc tardo:"`cat tiempo.txt`" (h:m:s)"
if [ $estado_programa -ne 0 ]; then
        # Actualizar los registros de eventos
        actualizar_registro "***ERROR*** No se pudo ejecutar correctamente el modelo ADCIRC para el dominio "$DOMINIO

        # Finalizar el script con error
        exit 1
fi

actualizar_registro "Fin de la ejecucion de padcirc para el dominio "$DOMINIO
